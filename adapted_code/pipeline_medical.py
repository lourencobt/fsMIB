...
def make_multisource_episode_pipeline(dataset_spec_list,
  ...
  sources = []
  # chunk_sizes_list = []
  for source_id, (dataset_spec, episode_descr_config, use_dag_ontology, use_bilevel_ontology,
                  num_to_take_for_dataset) in enumerate(
                      zip(dataset_spec_list, episode_descr_configs, use_dag_ontology_list,
    ...
    chunk_sizes_dataset = tf.data.Dataset.from_tensors(chunk_sizes).repeat()
    sources.append(tf.data.Dataset.zip((dataset, source_id_dataset, chunk_sizes_dataset)))
  ...
  def map_fn(episode, source_id, chunk_sizes):
...
def make_one_source_batch_pipeline(dataset_spec,
  ...
  dataset = batch_reader.create_dataset_input_pipeline(
      batch_size=batch_size, pool=pool, shuffle_seed=source_sampling_seed)
...
def make_multisource_batch_pipeline(dataset_spec_list,
  ...
  for source_id, (dataset_spec, num_to_take_for_dataset) in enumerate(
    ...
    batch_reader = reader.BatchReader(dataset_spec, split, shuffle_buffer_size,
                                      ...
                                      num_to_take_for_dataset)
    dataset = batch_reader.create_dataset_input_pipeline(
        batch_size=batch_size, pool=pool, offset=offset, shuffle_seed=source_sampling_seed)
    # Create a dataset to zip with the above for identifying the source.
    source_id_dataset = tf.data.Dataset.from_tensors(source_id).repeat()
  ...
  # Sample uniformly among sources
  dataset = tf.data.experimental.sample_from_datasets(sources, seed=source_sampling_seed)
